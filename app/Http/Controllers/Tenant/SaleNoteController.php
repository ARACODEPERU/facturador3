...
class SaleNoteController extends Controller
{
    ...
    public function store(SaleNoteRequest $request)
    {
        ...
        DB::connection('tenant')->transaction(function () use ($request) {
            ...
            $cash = Cash::where('user_id', $user_id)->where('state', true)->first();
            if ($cash) {
                $cash_id = $cash->id;
            } else {
                return [
                    'success' => false,
                    'message' => "Debe tener una caja activa",
                ];
            }
            ...
            $this->saveCashDocument($cash_id, $this->sale_note->id);
    ...
    public function saveCashDocument($id, $sale_note_id)
    {
    ...
    public function getFullDescription($row, $warehouse)
    {
        ...
        if ($row->unit_type_id != 'ZZ') {
            if ($row->warehouses && $warehouse) {
                $item_ers = $row->warehouses->where('warehouse_id', $warehouse->id)->first();
                if ($item_ers) {
                    $warehouse_stock = number_format($item_ers->stock, 2);
                } else {
                    $warehouse_stock =  0;
                }
            } else {
